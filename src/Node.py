...
class Node:
    """
        ...
        * parameter(s): 
            ...
            - isFood: a boolean flag that idicates whether this node contains a food object. i.e. true/False
    """

    def __init__(self, index, isWall=False, isExit=False, isEnemy=False, isFood=False):
        x = index[0]
        y = index[1]
        self.__index = (x, y)
    ...
    def isEqual(self, another):
        if self.__index == another.index and self.isWall == another.isWall and self.isExit == another.isExit and self.isFood == another.isFood:
            return True
        else:
            return False
